...
import { AlertTriangle, Activity, Clock, Database, Pause, Play, RefreshCw, Server } from 'lucide-react';
      ...
      >
        <div>
          ...
          <div style={{ color: c.textSecond, marginTop: '6px', fontSize: '14px' }}>
            Area {areaCodeParam ?? summary?.areaCode ?? 'all'} · recent window{' '}
            {summary?.recentWindowMinutes ?? recentMinutes} min
          </div>
        </div>
      ...
      {!loading && !error && summary && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          <section style={gridStyle('repeat(auto-fit, minmax(180px, 1fr))')}>
            <MetricCard
              title="Run"
              value={displayRun?.status ?? 'No run'}
              detail={displayRun?.scenarioCode ?? 'No simulation run found'}
              colors={c}
              tone="#2563eb"
            ...
            <MetricCard
              title="Inbox"
              value={summary.pipeline.inboxTotal}
              detail={`${summary.pipeline.inboxRecent} recent`}
              colors={c}
              tone="#059669"
            ...
            <MetricCard
              title="Attempts"
              value={summary.pipeline.attemptsRecent}
              detail="recent processing attempts"
              colors={c}
              tone="#7c3aed"
              icon={<Server size={18} />}
            />
            <MetricCard
              title="Rejected"
              value={summary.pipeline.rejectedRecent}
              detail={`${summary.pipeline.rejectedTotal} total`}
              colors={c}
              tone="#dc2626"
            ...
            <MetricCard
              title="Quarantined"
              value={summary.pipeline.quarantinedRecent}
              detail={`${summary.pipeline.quarantinedTotal} total`}
              colors={c}
              tone="#ea580c"
              icon={<AlertTriangle size={18} />}
            />
            <MetricCard
              title="Risk"
              value={summary.risk.recentCount}
            ...
            <MetricCard
              ...
              detail="fresh / stale / expired cell states"
              colors={c}
              tone="#475569"
              icon={<Clock size={18} />}
            />
          ...
          <Panel colors={c}>
            <SectionHeader
              title="Current / Latest Run"
              subtitle="Read from persisted control.simulation_runs metadata"
            />
          ...
          <Panel colors={c}>
            <SectionHeader
              title="Runtime Chain"
              subtitle="Counts come from persisted projections and pipeline tables"
            />
              ...
              {chain.map((item, index) => (
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ ...chainBlock(c, item.tone), flex: 1 }}>
                    ...
                    <div style={{ fontSize: '12px', color: item.warning ? '#dc2626' : c.textSecond, marginTop: '2px' }}>
                      {item.status}
                    </div>
                  </div>
          ...
          <section style={gridStyle('repeat(auto-fit, minmax(280px, 1fr))')}>
            <ChartPanel title="Inbox by Status" colors={c}>
              <BarGraph
                data={summary.pipeline.inboxByStatus.map((item) => ({ name: item.status, value: item.count }))}
                color="#059669"
              />
            </ChartPanel>
          ...
          <Panel colors={c}>
            <SectionHeader
              title="Risk Scores"
              subtitle="Recent persisted risk_assessment_log values, no frontend scoring"
            />
            {summary.risk.recentScores.length === 0 ? (
              ...
              <div style={{ height: '240px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={summary.risk.recentScores.map((point) => ({
          ...
          <Panel colors={c} accent="#b45309">
            ...
            <ul style={{ margin: 0, paddingLeft: '18px', color: c.textSecond, lineHeight: 1.7 }}>
              ...
              {summary.warnings.map((item) => (
                <li key={item} style={{ color: '#b45309' }}>
                  {item}
                </li>
              ))}
            </ul>
...
function RunDetails({ run, colors }: { run: RuntimeRunSummaryResponse | null; colors: ReturnType<typeof getColors> }) {
  ...
  return (
    <div style={gridStyle('repeat(auto-fit, minmax(190px, 1fr))')}>
      ...
      <KeyValue
        label="Duration"
        value={run.durationSeconds == null ? 'running/unknown' : `${Math.round(run.durationSeconds)}s`}
        colors={colors}
      />
      ...
      <KeyValue
        ...
        value={run.runOverrides?.selectedSensorNames.join(', ') || 'not in metadata'}
        colors={colors}
      />
...
function RejectedList({
  ...
  items: RuntimeRejectedEventResponse[];
  colors: ReturnType<typeof getColors>;
}) {
  if (items.length === 0) {
...
function QuarantineList({
  ...
  items: RuntimeQuarantinedEventResponse[];
  colors: ReturnType<typeof getColors>;
}) {
  if (items.length === 0) {
    ...
    <ListBox>
      {items.map((item) => (
        <Row key={item.id} colors={colors}>
          ...
          <small>
            {formatDate(item.quarantinedAt)} · attempt {item.finalAttemptNumber}
          </small>
        </Row>
      ))}
    </ListBox>
...
function AttemptList({
  ...
  items: RuntimeProcessingAttemptResponse[];
  colors: ReturnType<typeof getColors>;
}) {
  if (items.length === 0) {
  ...
  return (
    <ListBox>
      {items.map((item) => (
        <Row key={item.id} colors={colors}>
          <strong>
            {item.outcome} · {item.errorCode ?? 'no error code'}
          </strong>
...
function Panel({
  ...
  return (
    <section
      style={{ ...panelStyle(colors), borderTop: accent ? `3px solid ${accent}` : `1px solid ${colors.panelBorder}` }}
    >
      {children}
    </section>
  );
...
function buildRuntimeChain(summary: RuntimeSummaryResponse | null) {
  ...
  return [
    {
      label: 'Run',
      value: summary?.currentRun ? 'active' : summary?.latestRun ? 'latest' : 'none',
      status: summary?.latestRun?.status ?? 'not observed',
      warning: !summary?.latestRun,
      tone: '#2563eb',
    },
    {
      label: 'Inbox',
      value: pipeline?.inboxTotal ?? 0,
      status: `${pipeline?.inboxRecent ?? 0} recent`,
      warning: false,
      tone: '#059669',
    },
    ...
    {
      label: 'Cell states',
      value: summary?.cellOperationalStateCount ?? 0,
      status: summary?.areaOperationalState ? 'projection rows updated' : 'no area state',
      warning: !summary?.areaOperationalState,
      tone: '#be123c',
    },
    {
      label: 'Alerts',
    ...
    {
      label: 'API',
      value: summary ? 'ok' : 'n/a',
      status: summary ? 'summary loaded' : 'not loaded',
      warning: !summary,
      tone: '#475569',
    },
  ];
...
function formatOverrides(values: RuntimeRunOverrideValuesResponse | null | undefined) {
  ...
  const parts = [
    ...
    values.degradationProfiles && values.degradationProfiles.length > 0
      ...
      : values.degradationProfile == null
        ? null
        : values.degradationProfile,
  ].filter(Boolean);
