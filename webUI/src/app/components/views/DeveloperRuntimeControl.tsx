...
export function DeveloperRuntimeControl({ isDark }: { isDark: boolean }) {
  ...
  const activeSensorCount = useMemo(() => sensorNodes.filter((sensor) => sensor.isActive).length, [sensorNodes]);
  ...
  const scenarioCWithoutDegradation =
    runForm.scenarioCode === 'scenario_c' && activeDegradationProfiles.every((profile) => profile === 'none');
    ...
    <main style={{ minHeight: '100vh', background: c.pageBg, color: c.textPrimary, padding: '24px' }}>
      <section style={topBar()}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          />
          <input
            style={{ ...input(c), width: '90px' }}
            type="number"
            value={recentMinutes}
            onChange={(event) => setRecentMinutes(Number(event.target.value))}
            aria-label="Recent minutes"
          />
          <button type="button" style={button(c)} onClick={loadBase} disabled={loading}>
            <RefreshCw size={16} /> Refresh
          </button>
          <a style={button(c)} href={`/dashboards/${areaCode}/pipeline`}>
            Runtime Monitor
          </a>
        </div>
      </section>
      ...
      <section style={grid('repeat(auto-fit, minmax(180px, 1fr))')}>
        ...
        <Metric
          colors={c}
          title="Latest run"
          value={summary?.latestRun?.status ?? 'none'}
          detail={summary?.latestRun?.scenarioCode ?? 'no run'}
        />
        <Metric colors={c} title="Missing events" value={runAudit?.missingEvents ?? 'n/a'} detail="latest run audit" />
        <Metric
          colors={c}
          title="Risk assessments"
          value={runAudit?.riskAssessments ?? 'n/a'}
          detail="latest run audit"
        />
        <Metric
          colors={c}
          title="Freshness"
          value={
            summary?.freshness
              ? `${summary.freshness.freshCount}/${summary.freshness.staleCount}/${summary.freshness.expiredCount}`
              : 'n/a'
          }
          detail="fresh/stale/expired"
        />
      </section>
      ...
      {runAudit && (
        <Panel colors={c} accent="#2563eb">
          <SectionTitle
            ...
            subtitle={`${runAudit.run.scenarioCode} · ${runAudit.run.runOverrides?.resolved?.degradationProfile ?? 'degradation unknown'}`}
          />
          <ResultTable
            colors={c}
            result={{
              ...
              rows: [
                ...
                {
                  metric: 'qualityFlags',
                  value: runAudit.qualityFlagsSummary.map((item) => `${item.status}:${item.count}`).join(', '),
                },
                {
                  metric: 'eligibility',
                  value: runAudit.eligibilitySummary.map((item) => `${item.status}:${item.count}`).join(', '),
                },
                {
                  metric: 'areaSnapshot',
                  value: runAudit.areaSnapshot
                    ? `${runAudit.areaSnapshot.aggregateRiskLevel} ${runAudit.areaSnapshot.aggregateRiskScore}`
                    : '',
                },
              ],
      ...
      <Panel colors={c}>
        <SectionTitle
          title="Diagnostics"
          subtitle="Buttons execute fixed backend diagnostics. No free-form SQL is accepted."
        />
        <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '16px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {diagnostics.map((item) => (
              <button
                type="button"
                key={item.id}
                style={button(c, selectedDiagnostic === item.id)}
          ...
          <div>
            <SectionTitle
              title={diagnosticResult?.title ?? activeDiagnostic?.title ?? 'Diagnostic result'}
              subtitle={diagnosticResult?.description ?? activeDiagnostic?.description}
            />
      ...
      <section style={grid('repeat(auto-fit, minmax(360px, 1fr))')}>
        <Panel colors={c}>
          <SectionTitle
            ...
            subtitle="Starts Simulator.Host in Development with RunOverrides. Parallel runs are blocked by default."
          />
          <FormGrid>
            <LabeledSelect
              colors={c}
              ...
              options={scenarios.map((scenario) => ({
                value: scenario.code,
                label: `${scenario.code} - ${scenario.name} (${scenario.scenarioKind})`,
              }))}
              onChange={(value) =>
                setRunForm({
                  ...runForm,
                  scenarioCode: value,
                  degradationProfile:
                    value === 'scenario_c' && runForm.degradationProfile === 'none'
                      ? 'missing-readings'
                      : runForm.degradationProfile,
                  degradationProfiles:
                    value === 'scenario_c' && activeDegradationProfiles.every((profile) => profile === 'none')
                      ? ['missing-readings']
                      : activeDegradationProfiles,
            ...
            <LabeledNumber
              ...
              onChange={(value) => setRunForm({ ...runForm, sensorCount: value })}
            />
            <LabeledNumber
              colors={c}
              label="numberOfCycles"
              value={runForm.numberOfCycles}
              onChange={(value) => setRunForm({ ...runForm, numberOfCycles: value })}
            />
            <LabeledNumber
              colors={c}
              label="intervalSeconds"
              value={runForm.intervalSeconds}
              onChange={(value) => setRunForm({ ...runForm, intervalSeconds: value })}
            />
            <LabeledNumber
              colors={c}
              label="seed"
              value={runForm.seed}
              onChange={(value) => setRunForm({ ...runForm, seed: value })}
            />
            <LabeledNumber
              colors={c}
              label="timeoutSeconds"
              value={runForm.timeoutSeconds}
              onChange={(value) => setRunForm({ ...runForm, timeoutSeconds: value ?? 180 })}
            />
            <LabeledInput
              colors={c}
              label="runLabel"
              value={runForm.runLabel ?? ''}
              onChange={(value) => setRunForm({ ...runForm, runLabel: value || null })}
            />
          </FormGrid>
          <div style={{ color: c.textSecond, fontSize: '13px', marginTop: '10px' }}>
            ...
            {runForm.sensorCount ?? 'all'}
          </div>
          <div style={{ marginTop: '10px' }}>
            <div style={label(c)}>degradation profiles</div>
          ...
          {sensorCountTooHigh && (
            <InlineWarning colors={c}>
              sensorCount {runForm.sensorCount} exceeds {activeSensorCount} active sensor(s). Lower the value before
              submitting.
            </InlineWarning>
          )}
          {scenarioCWithoutDegradation && (
            <InlineWarning colors={c}>
              scenario_c is intended for degraded/operational comparison. With degradationProfile=none it may behave
              like a clean scenario.
            </InlineWarning>
          )}
          <CheckRow
            colors={c}
            ...
            onChange={(value) => setRunForm({ ...runForm, collectEvidence: value })}
          />
          <CheckRow
            colors={c}
            label="waitForCompletion"
            checked={runForm.waitForCompletion}
            onChange={(value) => setRunForm({ ...runForm, waitForCompletion: value })}
          />
          <CheckRow
            colors={c}
            label="allowParallelRun"
            checked={runForm.allowParallelRun}
            onChange={(value) => setRunForm({ ...runForm, allowParallelRun: value })}
          />
          <button
            type="button"
            style={{ ...button(c), marginTop: '12px', opacity: submittingRun || sensorCountTooHigh ? 0.65 : 1 }}
            onClick={startRun}
            disabled={submittingRun || sensorCountTooHigh}
          >
        ...
        <Panel colors={c} accent="#dc2626">
          <SectionTitle
            ...
            subtitle="Dry run first. Real reset requires exact confirmation and blocks active runs."
          />
          <button type="button" style={button(c)} onClick={() => resetRuntime(true)} disabled={loading}>
            <Search size={16} /> Dry run reset
          </button>
          <div style={{ marginTop: '12px' }}>
            <label style={label(c)} htmlFor="runtime-reset-confirmation">
              Confirmation
            </label>
            <input
              id="runtime-reset-confirmation"
              style={input(c)}
              value={confirm}
          ...
          <button
            type="button"
            style={{ ...button(c), borderColor: '#dc2626', color: '#dc2626', marginTop: '12px' }}
      ...
      <Panel colors={c} accent="#64748b">
        ...
        <ul style={{ color: c.textSecond, lineHeight: 1.7, margin: 0, paddingLeft: '20px' }}>
          ...
          <li>
            Area Operational State uses persisted projections and may include carry-forward. It is not necessarily
            limited to the latest run.
          </li>
        </ul>
      </Panel>
    </main>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tbody>
            {result.rows.length === 0 ? (
              <tr>
                <td style={cell(colors)} colSpan={Math.max(1, result.columns.length)}>
                  No rows
                </td>
              </tr>
            ) : (
              result.rows.map((row, index) => (
                <tr key={result.columns.map((column) => `${column}:${String(row[column] ?? '')}`).join('|')}>
                  {result.columns.map((column) => (
...
function LabeledInput({
  ...
  return (
    <label style={{ display: 'grid', gap: '4px' }}>
      <span style={label(colors)}>{text}</span>
      <input aria-label={text} style={input(colors)} value={value} onChange={(event) => onChange(event.target.value)} />
    </label>
  );
}
...
function LabeledSelect({
  ...
  return (
    <label style={{ display: 'grid', gap: '4px' }}>
      <span style={label(colors)}>{text}</span>
      <select aria-label={text} style={input(colors)} value={value} onChange={(event) => onChange(event.target.value)}>
...
function CheckRow({
  ...
  return (
    <label style={{ display: 'flex', gap: '8px', alignItems: 'center', marginTop: '10px', color: colors.textSecond }}>
      <input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /> {text}
    </label>
  );
}
...
function RunRequestResult({
  ...
  return (
    ...
    >
      <SectionTitle
        ...
        subtitle={
          request.waitForCompletion
            ? 'Waiting for terminal status when requested.'
            : 'Run submitted. Follow live processing in Runtime Monitor.'
        }
      />
      <ResultTable
        colors={colors}
      ...
      />
      <a style={{ ...button(colors), marginTop: '10px' }} href={`/dashboards/${areaCode}/pipeline`}>
