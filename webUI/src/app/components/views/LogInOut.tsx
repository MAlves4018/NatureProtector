      ...
      <section style={cardStyle(c, isPanel)}>
        ...
        {signedIn ? (
          <div style={{ marginTop: '18px', display: 'grid', gap: '12px' }}>
            <button type="button" onClick={handleLogout} disabled={loading} style={primaryButton(c, '#ef4444')}>
              <LogOut size={16} /> Sign out
            </button>
          </div>
        ) : (
          <div style={{ marginTop: '18px', display: 'grid', gap: '12px' }}>
            <label style={labelStyle(c)} htmlFor="usernameOrEmail">
              Username or email
            </label>
            <input
              id="usernameOrEmail"
            ...
            />
            <label style={labelStyle(c)} htmlFor="password">
              Password
            </label>
            <input
              id="password"
        ...
        {error && <div style={{ marginTop: '12px', color: c.redText, fontSize: '13px' }}>{error}</div>}
      </section>
    </div>
  );
}
