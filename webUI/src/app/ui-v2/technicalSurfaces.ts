...
import { formatUiV2Date, translate, UiV2Locale } from './i18n';

export type UiV2TechnicalState =
  | 'ready'
  | 'partial'
  | 'blocked'
  | 'unknown'
...
export interface UiV2TechnicalField {
  label: string;
  value: string;
  state: UiV2TechnicalState;
  source: string;
  timestamp: string;
  scope: string;
  limitation: string;
...
export interface UiV2PipelineSurface {
  state: UiV2TechnicalState;
  summary: string;
  fields: UiV2TechnicalField[];
  limitations: string[];
}

export interface UiV2QaSuite {
  suiteId: string;
  suiteName: string;
  category: string;
...
export interface UiV2EvidenceItem {
  evidenceId: string;
  title: string;
  type: string;
  source: string;
  ...
  availability: UiV2TechnicalState;
  reference: string;
  limitations: string[];
}

export interface UiV2AdminAction {
  capability: string;
  action: string;
...
export interface UiV2P3Surface {
  objective: string;
  status: string;
...
export interface UiV2ReadinessItem {
  item: string;
  status: UiV2TechnicalState;
  evidence: string;
  limitation: string;
}

export function buildUiV2PipelineSurface(
  input: {
  ...
  const generatedAt = formatUiV2Date(summary?.generatedAtUtc, locale);
  const runTimestamp = formatUiV2Date(run?.endedAt ?? run?.startedAt ?? run?.createdAt, locale);
...
export function buildUiV2QaSuites(): UiV2QaSuite[] {
  return [
    {
      suiteId: 'm04-ui-v2-focused',
      suiteName: 'UI v2 focused frontend tests',
      category: 'component, i18n, accessibility smoke, capability UX',
      testDefinition: 'npm test -- src/app/ui-v2 src/app/services/api.test.ts',
      ...
      coverage: 'app/ui-v2 81.28% lines',
    ...
    },
    {
      ...
      limitations: [
        ...
        'Targeted UI v2 scan found only test-token/password test fixture strings and technical authorization labels.',
      ],
    },
  ];
}

export function buildUiV2EvidenceItems(
  input: {
    summary: RuntimeSummaryResponse | null;
    run: RuntimeRunSummaryResponse | SimulationRunResponse | null;
    audit: RuntimeRunAuditResponse | null;
  ...
  return [
    ...
    },
    {
      ...
      title: 'Runtime summary loaded by UI v2',
...
export function buildUiV2AdminActions(user: Pick<User, 'roles'> | null | undefined): UiV2AdminAction[] {
  ...
  return [
    ...
    },
    {
      ...
      riskLevel: 'High',
      authorizationState: 'Backend endpoint exists for Sim/Admin in Development; not exposed by M05 UI',
      ...
      limitations: ['Not exposed because preserving M04 smoke state is required and reset can be destructive.'],
    },
    {
      capability: 'admin.execute',
...
export function buildUiV2P3Surface(
...
export function buildUiV2ReadinessItems(input: {
  summary: RuntimeSummaryResponse | null;
  run: RuntimeRunSummaryResponse | SimulationRunResponse | null;
...
function queueMetricState(queue: RabbitMqQueueMetricResponse | null | undefined): UiV2TechnicalState {
  if (!queue) {
    return 'unknown';
  }
...
function valueForState(_state: UiV2TechnicalState) {
  return '-';
}

function notAvailable(locale: UiV2Locale) {
  return translate(locale, 'value.notAvailable');
}

function notConfirmed(locale: UiV2Locale) {
  return translate(locale, 'value.notConfirmed');
}

function noEvidence(locale: UiV2Locale) {
  return translate(locale, 'value.noEvidence');
}

function noRecentExecution(locale: UiV2Locale) {
  return translate(locale, 'value.noRecentExecution');
}

function none(locale: UiV2Locale) {
