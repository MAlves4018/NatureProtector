...
export function StatusBadge({ label, state = 'ready' }: { label: string; state?: UiV2OutputState | UiV2TechnicalState | 'ready' }) {
  const tone = state === 'blocked' || state === 'error' || state === 'access-denied' || state === 'no-evidence'
    ? 'ui-v2-badge-error'
