import { Activity, Cloud, FileCheck2, GitBranch, ShieldCheck } from 'lucide-react';
  ...
  return (
    <section className="ui-v2-page">
      <PageHeader
        ...
        helpTopic="qa"
      />
      <ul className="ui-v2-mission-flow" aria-label="Engineering lifecycle">
      ...
      <section aria-labelledby="release-readiness-title">
        <div className="ui-v2-section-heading">
          <div>
            <p className="ui-v2-kicker">Derivado de estados e evidence</p>
            <h2 id="release-readiness-title">Release readiness</h2>
          </div>
        </div>
        <div className="ui-v2-readiness-grid">
          {readiness.map((item) => (
            <article className="ui-v2-card ui-v2-readiness-card" key={item.label}>
              <div className="ui-v2-section-heading">
                <strong>{item.label}</strong>
                <span className="ui-v2-risk" data-risk={item.risk}>
                  {item.status}
                </span>
              </div>
              <p>{item.detail}</p>
      ...
      <div className="ui-v2-section-heading">
        ...
        <button type="button" className="ui-v2-secondary" onClick={() => void refresh()} disabled={loading}>
          {loading ? 'A atualizar…' : 'Atualizar'}
        </button>
      </div>
      {error && <p className="ui-v2-notice ui-v2-error">{error.message}</p>}
      {latest.length === 0 ? (
        <p className="ui-v2-notice">Ainda não existem operações registadas neste store.</p>
      ) : (
...
function MissionNode({ icon, label, detail }: { icon: ReactNode; label: string; detail: string }) {
  return (
    <li className="ui-v2-mission-node">
      {icon}
      <strong>{label}</strong>
      <span>{detail}</span>
    </li>
