    ...
    <section className="ui-v2-page">
      ...
      <section className="ui-v2-card">
        ...
        <div className="ui-v2-key-values">
          ...
          <span><strong>Evidence existente</strong>{p3Surface.existingEvidence}</span>
          <span><strong>Readiness</strong>{p3Surface.readiness}</span>
          <span><strong>Next gate</strong>{p3Surface.nextGate}</span>
        </div>
      </section>
      <TechnicalDetail title="Detalhe P3" fields={p3Surface.fields} />
      <section className="ui-v2-notice">
        <strong>Limites P3</strong>
        <ul>{p3Surface.limitations.map(item => <li key={item}>{item}</li>)}</ul>
      </section>
    </section>
  );
}
