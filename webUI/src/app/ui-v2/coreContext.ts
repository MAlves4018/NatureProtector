...
export type UiV2AreaSelectionStatus =
  | 'not-selected'
  | 'resolving'
  | 'resolved'
  | 'unavailable'
...
export type UiV2RunLifecycleState =
  | 'not-selected'
  | 'not-found'
  | 'pending'
  | 'running'
...
export type UiV2ScenarioAvailability = 'not-selected' | 'available' | 'not-found' | 'unavailable' | 'error';

export interface UiV2AreaResolutionModel {
  requestedArea: string | null;
  resolvedArea: AreaResponse | null;
  availableAreas: AreaResponse[];
  selectionStatus: UiV2AreaSelectionStatus;
  resolutionStatus: UiV2AreaSelectionStatus;
  resolutionReason: string;
}

export interface UiV2RunContextModel {
  requestedRunId: string | null;
  resolvedRunId: string | null;
  run: RuntimeRunSummaryResponse | SimulationRunResponse | null;
  state: UiV2RunLifecycleState;
  fields: Array<{ label: string; value: string }>;
  limitations: string[];
}

export interface UiV2ScenarioContextModel {
  requestedScenarioId: string | null;
  resolvedScenarioId: string | null;
  scenario: ScenarioResponse | null;
  availability: UiV2ScenarioAvailability;
  fields: Array<{ label: string; value: string }>;
  limitations: string[];
}

export interface UiV2SimulationReviewModel {
...
export function resolveUiV2Area(
  requestedArea: string | null | undefined,
  availableAreas: AreaResponse[],
  locale: UiV2Locale,
  loading = false,
  error: Error | null = null,
): UiV2AreaResolutionModel {
...
export function buildUiV2RunContext(
  input: {
    requestedRunId?: string | null;
  ...
  return {
    ...
    fields: [
      ...
      { label: coreLabel(locale, 'created'), value: formatUiV2Date(run.createdAt, locale) },
      { label: coreLabel(locale, 'started'), value: formatUiV2Date(run.startedAt, locale) },
...
export function buildUiV2ScenarioContext(
  scenarioCode: string | null | undefined,
  scenarios: ScenarioResponse[],
  locale: UiV2Locale,
  error: Error | null = null,
): UiV2ScenarioContextModel {
...
export function buildUiV2SimulationReview(
  request: RuntimeRunStartRequest,
  result: RuntimeRunStartResponse | null,
  locale: UiV2Locale,
): UiV2SimulationReviewModel {
...
function mapRunState(status: string): UiV2RunLifecycleState {
...
function formatMaybe(value: unknown, locale: UiV2Locale) {
...
function coreLabel(locale: UiV2Locale, key: CoreLabelKey) {
  const labels: Record<UiV2Locale, Record<CoreLabelKey, string>> = {
    'pt-PT': {
