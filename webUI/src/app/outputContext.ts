...
import { formatUiDate, formatUiNumber, translate, UiLocale, UiMessageKey } from './i18n';

export type UiOutputState =
  | 'loading'
  | 'ready'
  | 'no-data'
  | 'partial'
...
export interface UiRiskReadModel {
  state: UiOutputState;
  area: string;
  run: string;
  ...
  summary: string;
  contextFields: UiContextField[];
  limitations: string[];
  warnings: string[];
  canShowScore: boolean;
}

export interface UiRiskReadInput {
...
const unknown = (locale: UiLocale) => translate(locale, 'value.unknown');
const notAvailable = (locale: UiLocale) => translate(locale, 'value.notAvailable');

export function buildUiRiskReadModel(input: UiRiskReadInput, locale: UiLocale): UiRiskReadModel {
  ...
  const state: UiOutputState = blocked
    ? 'blocked'
...
function emptyModel(state: UiOutputState, locale: UiLocale): UiRiskReadModel {
  return {
    state,
    area: unknown(locale),
...
function buildContextFields(
  ...
  return [
    ...
    field(
      'temporal',
      'status.temporal',
      `${formatUiDate(summary.generatedAtUtc, locale)} (${summary.recentWindowMinutes}m)`,
      'ready',
      'help.freshness',
    ),
    field(
...
function freshnessCounts(summary: RuntimeSummaryResponse, locale: UiLocale) {
...
function stateFromText(value: string | null | undefined): UiOutputState {
