...
\texttt{pipeline.}\newline\texttt{event\_inbox} & Eventos recebidos e respetivo estado de processamento. \\
\texttt{pipeline.}\newline\texttt{processing\_attempts} & Tentativas de processamento, resultados e erros. \\
\texttt{pipeline.}\newline\texttt{rejected\_events} & Eventos rejeitados antes da pipeline de risco aceite. \\
\texttt{pipeline.}\newline\texttt{quarantined\_events} & Eventos com retry esgotado ou estado não recuperável. \\
\texttt{projection.}\newline\texttt{accepted\_reading\_log} & Leituras aceites na pipeline de risco. \\
\texttt{projection.}\newline\texttt{risk\_assessment\_log} & Avaliações candidatas de risco. \\
\texttt{projection.}\newline\texttt{cell\_operational\_state} & Estado operacional por célula. \\
\texttt{projection.}\newline\texttt{area\_operational\_state} & Estado operacional agregado por área. \\
\texttt{projection.}\newline\texttt{alert\_state} & Estado de alerta produzido pela política interna. \\
\hline
\end{tabular}
\end{table}
