...
{%
  ...
  {\HeaderSmall\color{Fire}\textbf{A proteger florestas para gerações futuras} \hspace{3mm} · \hspace{3mm} Ideia reconhecida com o \textbf{3.º lugar} no concurso de startups IdeaUP 2025}
};
...
Na fase Beta, o maior risco é mostrar \Key{resultados sem contexto}. Um valor isolado não explica \Key{origem dos dados}, \Key{tratamento na pipeline} e \Key{confiabilidade do estado}. O projeto precisa de evidência técnica para justificar cada leitura e cada decisão.%
}
...
O foco é ter um \OrangeKey{sistema end-to-end}:
leituras simuladas entram como eventos, são validadas,
persistidas e convertidas em projeções.

É essencial conhecer e apresentar visualmente os valores de cada etapa
e o momento em que cada etapa ocorre,
não apenas o resultado final (risco de incêndio). 

Este projeto é uma base que será expandida e melhorada com dados reais, em tempo real,
integração com sistemas de emergência e proteção civil (tal como estações de bombeiros)
e eventual uso de machine learning para avaliações de risco mais sofisticadas.
