...
## 18. Maximização de testes, qualidade e CI/CD

Foi executada uma missão de hardening de testes, qualidade, supply chain e CI/CD sem alterar `.env`, `.env.example`, contratos RabbitMQ, scoring, semântica de alertas, roles, claims, P3, schemas ou migrations. O foco prático ficou nas fases F0/F1 e em dois guardrails pequenos de F2.

A baseline atual foi revalidada em Release. `dotnet restore .\NatureProtector.sln --configfile NuGet.Config`, `dotnet build .\NatureProtector.sln -c Release --no-restore --nologo -v minimal` e `dotnet test .\NatureProtector.sln -c Release --no-build --no-restore --nologo -v minimal` passaram. A suite backend passou `1192/1192` testes, sem skipped, depois de acrescentar 3 testes de arquitetura para impedir dependências indevidas de `Core`, `Shared` e `Prevention`.

No frontend, `npm ci`, `npm run typecheck`, `npm run test:coverage`, `npm run build` e `npm run audit:ci` passaram. A suite passou `35/35` testes em 9 ficheiros. Foi corrigida a incompatibilidade observada em Node 22 nos testes de `LoggedOutBlock`: os imports de routing foram alinhados em `react-router-dom`, a dependência direta `react-router` foi removida do root, e foi criado um teste estático para bloquear novos imports diretos de `react-router` em `webUI/src/app`. A validação com `npx -y -p node@22.16.0 -p npm@10.8.2 npm test` passou `35/35`.

O advisory NuGet moderado `NU1902` de `OpenTelemetry.Exporter.OpenTelemetryProtocol 1.10.0` foi removido por atualização coesa dos pacotes OpenTelemetry estáveis e do pacote beta de process instrumentation para a linha atual compatível. `.\scripts\ci\check-dotnet-audit.ps1 -OutputPath .\artifacts\dotnet-audit.txt` passou e reportou zero pacotes vulneráveis nos projetos atuais.

A auditoria npm continua a reportar 3 advisories high em tooling/transitivo (`@vitejs/plugin-react`, `vite`, `esbuild`). Não foi executado `npm audit fix --force`. Foi criado `webUI/scripts/check-npm-audit.mjs` para tornar a política executável: critical ou high runtime direto bloqueiam, high tooling/transitivo fica visível como gate não bloqueante até haver upgrade compatível ou decisão de owner.

A workflow `.github/workflows/engineering-foundations.yml` foi endurecida preservando os nomes públicos dos checks existentes. Foram adicionados `permissions: contents: read`, `concurrency` com cancelamento, timeouts, actions pinadas por SHA, restore de tools, build Release, testes backend com TRX e Cobertura, relatório backend com ReportGenerator, upload de artefactos com retenção finita, audit NuGet, frontend sem execução duplicada da mesma suite, audit npm por script e um job adicional `Frontend Node 22 compatibility`.

O script `scripts/tests/generate-coverage-report.ps1` passou a restaurar o tool local `dotnet-reportgenerator-globaltool`, a invocar ReportGenerator via `dotnet tool run` e a preservar exit code de `dotnet test` e do relatório. Durante a validação foi detetado que processos locais antigos (`NatureProtector.Backoffice.Api` e `NatureProtector.Prevention.Host`) bloqueavam DLLs; os processos foram parados e o script foi reexecutado com sucesso.

Evidence gerada localmente: `TestResults/` com TRX e Cobertura, `coveragereport_backend/Summary.txt` com `29.5%` line coverage, `64.3%` branch coverage e `84.1%` method coverage no modo amplo de CI, e `coveragereport_core/Summary.txt` com `80%` line coverage, `66.5%` branch coverage e `88%` method coverage no script local filtrado. Estes valores são coverage técnico, não validação científica do modelo ou dos parâmetros candidatos.

Validações adicionais: YAML da workflow validado com `npx -y yaml@2 valid`; `graphify-out` não existia, por isso `graphify update .` foi saltado; `actionlint` e Go não estavam disponíveis no ambiente local. Não foi executada validação externa real de GitHub Actions, branch protection, service containers, browser E2E, Playwright, mutation testing, performance, release promotion ou deployment.
## 19. Continuacao da maximizacao de testes, qualidade e CI/CD

Foi executada a continuacao das fases tecnicas 2-7 da missao de qualidade, mantendo os limites de projeto: nao foram alterados `.env`, `.env.example`, contratos RabbitMQ publicados, schema/migrations, scoring, semantica de alertas, roles/claims ou integracao P3. As alteracoes continuam a tratar os parametros V1 como candidatos tecnicos, nao como calibracao cientifica final.

Na Fase 2, os mappings territoriais passaram a cobrir vocabulario real de hazard e land-cover, incluindo normalizacao de case, acentos e separadores. Foram adicionados testes table-driven e property-based para mappings, testes de DST/clock controlavel no classificador temporal, fixtures de compatibilidade JSON, metadata RabbitMQ de `contentType`/`contentEncoding`, matriz de autorizacao API, parity OpenAPI/frontend e guardrails de arquitetura/frontend para evitar drift, persistence leakage e secrets frontend. O mutation testing seletivo com Stryker foi materializado em `stryker-config.json` e executado sobre `TerritorialRiskContext` e `ReadingTemporalClassifier`, com score observado de `79.93%`.

Nas Fases 3-5, a integracao deixou de depender apenas de SQLite/mocks. Foram criados testes DockerIntegration para PostgreSQL real, RabbitMQ real e InfluxDB 3 real, incluindo inbox duravel, duplicate/retry/quarantine, publisher RabbitMQ com roundtrip de body e metadata, escrita/readback InfluxDB e um fluxo `ReadingRiskPipeline` com persistencia PostgreSQL e Influx. Tambem foram adicionados testes de recuperacao de leases expirados e quarantine por exaustao em PostgreSQL real. A workflow `engineering-foundations.yml` ganhou um job Docker integration com PostgreSQL, RabbitMQ e InfluxDB efemeros.

Na Fase 6, o frontend ganhou Playwright com Chromium, smoke publica `/ui-v2` e axe-core. O teste valida a superficie publica do produto, ausencia de navegacao protegida para anonimos e zero violacoes axe no percurso coberto. A falha inicial encontrou um problema real de contraste no botao de tema da navbar; a cor do texto foi elevada para o token primario e o E2E passou. O Vitest foi separado de `e2e/**` para manter unit/coverage e browser tests independentes. A CI frontend instala Chromium, corre `npm run test:e2e` e publica reports Playwright. Foi tambem acrescentada a workflow `security.yml` com CodeQL C#/JS-TS, dependency review em PR, audits NuGet/npm e scanner simples de canarios secretos em ficheiros versionados.

Na Fase 7, foi criado `benchmarks/NatureProtector.Benchmarks` com BenchmarkDotNet e perfis B0/B1/B2 para scoring, classificacao temporal, mappings territoriais e serializacao de envelopes. Foram adicionados scripts de release local: `build-release-candidate.ps1` publica API/Prevention/Simulator/Postgres Bootstrap, compila a webUI, gera inventarios de dependencias, checksums e zip; `test-clean-install.ps1` valida a estrutura do pacote; `test-postgres-backup-restore.ps1` cria bases temporarias, executa `pg_dump`/`pg_restore` e verifica restore com dados sentinela; `run-benchmarks.ps1` executa benchmarks parametrizaveis. A workflow `release-candidate.yml` materializa package, clean install e benchmark filtrado em execucao manual.
