...
| Lint frontend | `npm run lint` em `webUI/` | Executa Biome no scope frontend ratcheted: UI v2, services, e2e, scripts e config. Rewrites estilísticos/a11y legacy não estão ativos intencionalmente. |
...
| Browser integration frontend | `npm run test:e2e` em `webUI/` | Playwright constrói a app Vite, serve `dist/` via `vite preview` e executa browser checks contra o artifact construído. Chromium é a gate por defeito; definir `NP_PLAYWRIGHT_BROWSER_MATRIX=all` para Chromium, Firefox e WebKit. A spec autenticada UI v2 usa uma fixture HTTP bounded para exercitar jornadas Anonymous/Admin/Sim/Pipeline, estados de falha e download de evidence sem depender de uma base local live de identidades. |
